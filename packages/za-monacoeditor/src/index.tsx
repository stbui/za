...
function MonacoEditor(
    ...
    ref:
        | ((instance: RefEditorInstance) => void)
        | React.RefObject<RefEditorInstance>
        | null
        | undefined
) {
    const {
    ...
    useEffect(() => {
        if (editor.current) {
            ...
            (
                Object.keys(optionsRaw) as (keyof editor.IEditorOptions)[]
            ).forEach(keyname => {
                const propsOpt = options[keyname];
                if (
                    optionsRaw[keyname] !== propsOpt &&
                    propsOpt !== undefined
                ) {
...
export default React.forwardRef<RefEditorInstance, MonacoEditorProps>(
    MonacoEditor
);
